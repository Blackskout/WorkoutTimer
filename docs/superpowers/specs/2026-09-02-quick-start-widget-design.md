...
- **Приложение закрыто.** `NavHost` разбирает стартовый интент сам при создании графа.
- **Приложение уже открыто.** `MainActivity` в `onNewIntent()` вызывает `setIntent(intent)` и кладёт интент в `mutableStateOf`; `NavGraph` реагирует через `navController.handleDeepLink(intent)` и после обработки гасит значение (`null`).
