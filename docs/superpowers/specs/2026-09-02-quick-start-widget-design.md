...
- **Создание тренировки** по-прежнему ставит `lastUseAt = now`. Наверх виджета новая тренировка попадает не поэтому, а потому что у неё нет ни одной сессии (см. «Порядок» ниже); подпись честно скажет «ещё не делали».
...
**Порядок — от самой нужной к самой ненужной.** Сверху тренировки, которых нет в истории сессий, затем остальные по возрастанию `lastUseAt`: чем дольше не делали, тем выше, самая свежая — внизу. «Ещё не делали» определяется по отсутствию сессии, а не по `lastUseAt`: при создании тренировки туда пишется `now` (`CreateWorkoutViewModel.kt:114`), и по нему новая тренировка неотличима от сделанной только что — она уехала бы в самый низ. Побочный эффект: импорт не восстанавливает сессии (`ExportImportRepositoryImpl` их не пишет), поэтому сразу после импорта все тренировки попадают в верхнюю группу и упорядочиваются между собой по `lastUseAt`.
