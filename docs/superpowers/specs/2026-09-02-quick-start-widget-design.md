...
Виджет на домашнем экране со списком тренировок, отсортированных от самой нужной (дольше всего не делали) к самой свежей. Тап по строке открывает экран выполнения этой тренировки напрямую, минуя список.
...
- **Создание тренировки** ставит `lastUseAt = 0L` (`CreateWorkoutViewModel.kt`, задача 12 редизайна закрыла это на корню вместо `now`). Наверх виджета новая тренировка попадает именно поэтому — по возрастанию `lastUseAt` она меньше всех остальных (см. «Порядок» ниже); подпись честно скажет «ещё не делали».
...
**Порядок — от самой нужной к самой ненужной.** Простая сортировка по возрастанию `lastUseAt`: чем дольше не делали, тем выше, самая свежая — внизу. «Ещё не делали» определяется по `lastUseAt == 0L`, а не по отсутствию сессии: при создании тренировки туда пишется `0L` (`CreateWorkoutViewModel.kt`), это меньше значения любой уже выполненной тренировки, и новая тренировка естественно оказывается наверху сама по себе, без отдельного правила. Определять «ещё не делали» через отсутствие сессии не годится: тренировка, выполненная давно, до того как сессии стали записываться, сессии в истории не имеет, но «не деланной» не является — такое правило подняло бы её наверх ошибочно. Побочный эффект: импорт (`ExportImportRepositoryImpl.kt:116`) переносит `lastUseAt` из файла как есть, так что порядок после импорта не искажается.
